use dharitri_wasm::{Address, ArgBuffer, BoxedBytes, CodeMetadata, H256};
...
use dharitri_wasm::{BigIntApi, BigUintApi};
...
impl fmt::Display for TxInput {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "TxInput {{ func: {}, args: {:?}, call_value: {}, dct_token_name: {:?}, dct_value: {:?}, from: 0x{}, to: 0x{}\n}}", 
...
impl dharitri_wasm::ContractHookApi<RustBigInt, RustBigUint> for TxContext {
	...
	#[inline]
	fn get_dct_value_big_uint(&self) -> RustBigUint {
		self.tx_input_box.dct_value.clone().into()
	}

	#[inline]
	fn get_dct_token_name(&self) -> Vec<u8> {
		self.tx_input_box.dct_token_name.clone()
	}
...
impl dharitri_wasm::ContractIOApi<RustBigInt, RustBigUint> for TxContext {
