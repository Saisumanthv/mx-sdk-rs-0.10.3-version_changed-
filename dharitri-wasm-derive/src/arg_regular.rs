	...
	match &arg.ty {
		syn::Type::Reference(type_reference) => {
			...
			if let syn::Type::Slice(slice_type) = &*type_reference.elem {
				...
				quote! {
					dharitri_wasm::load_single_arg::<T, BigInt, BigUint, Box<[#slice_elem]>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			} else {
				...
				if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
					if let Some(ident) = path.get_ident() {
						if ident.to_string() == "str" {
							...
							return quote! {
								dharitri_wasm::load_single_arg::<T, BigInt, BigUint, Box<str>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
							};
						}
					}
				}
				...
				quote! {
					dharitri_wasm::load_single_arg::<T, BigInt, BigUint, #referenced_type>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			}
		},
		_ => {
			quote! {
				dharitri_wasm::load_single_arg::<T, BigInt, BigUint, #arg_ty>(self.api.clone(), #arg_index_expr, #arg_name_expr)
			}
		},
	}
}
