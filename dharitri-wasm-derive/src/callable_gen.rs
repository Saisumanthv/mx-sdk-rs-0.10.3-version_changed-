			...
			.map(|m| {
				...
				let (callback_init, callback_store) = if let Some(callback_ident) = &m.callback {
					...
					let callback_init = quote! {
						let mut callback_data_ser = dharitri_wasm::hex_call_data::HexCallDataSerializer::new( & #cb_name_literal );
					};
				...
				let sig = quote! {
					#msig {
						let mut call_data_ser = dharitri_wasm::hex_call_data::HexCallDataSerializer::new( & #m_name_literal );
						#callback_init
						#(#arg_push_snippets)*
						#callback_store
