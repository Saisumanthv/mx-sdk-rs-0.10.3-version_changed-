use dharitri_wasm::{Address, ArgBuffer, Box, BoxedBytes, CodeMetadata, Vec, H256};
...
extern {
    ...
    fn getDCTValue(resultOffset: *const u8) -> usize;
    fn getDCTTokenName(resultOffset: *const u8) -> usize;
...
impl dharitri_wasm::ContractHookApi<ArwenBigInt, ArwenBigUint> for ArwenApiImpl {
	#[inline]
	...
	fn get_dct_token_name(&self) -> Vec<u8> {
		...
		unsafe {
			...
			let name_len = getDCTTokenName(name.as_mut_ptr());
...
impl dharitri_wasm::ContractIOApi<ArwenBigInt, ArwenBigUint> for ArwenApiImpl {
	#[inline]
